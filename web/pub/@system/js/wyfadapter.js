...
jQuery.loadwyfobj = function(){
	if(jQuery.wyfobjnow<jQuery.wyfobjnum){
		var inphidden = $('[rel="wyf"]').eq(jQuery.wyfobjnow);
		var obj =  inphidden.attr('relobj');
		if(eval('"undefined" == typeof '+obj)){
			if(obj=='wyfcalendar'){
				getScript(wyf_jspath+'datePicker/WdatePicker.js',function(){
					jQuery.wyfobjnow = jQuery.wyfobjnow +1;
					jQuery.loadwyfobj();
				});
			}else if(obj=='kindeditor'){
				getScript(wyf_jspath+'kindeditor/kindeditor-min.js',function(){
					jQuery.wyfobjnow = jQuery.wyfobjnow +1;
					KindEditor.basePath = wyf_jspath+'kindeditor/';
					loadkindeditor(inphidden);
					jQuery.loadwyfobj();
				});
			}else if(obj=='wyfcolor'){
				getStyle(wyf_jspath+'jqcolor/jpicker.css');
				getScript(wyf_jspath+'jqcolor/jpicker.js',function(){
					jQuery.wyfobjnow = jQuery.wyfobjnow +1;
					jQuery.fn.jPicker.defaults.images.clientPath=wyf_jspath+'jqcolor/images/';
					loadjqcolor(inphidden);
					jQuery.loadwyfobj();

				});
			}else if(obj=='wyfmcalendar'){
				getStyle(wyf_jspath+'mobileDate/css/mobiscroll.custom-2.5.2.min.css');
				getScript(wyf_jspath+'mobileDate/js/mobiscroll.custom-2.5.2.min.js',function(){
					jQuery.wyfobjnow = jQuery.wyfobjnow +1;
				...
				getScript(wyf_jspath+'js/'+obj+'.js',function(){
					jQuery.wyfobjnow = jQuery.wyfobjnow +1;
					jQuery.loadwyfobj();
				});
			}
		...
		*/
		if(window.wyf_initfuns){
			for(var wyf_i=0;wyf_i<window.wyf_initfuns.length;wyf_i++){
				setTimeout(window.wyf_initfuns[wyf_i],0);
			}
		}
	}
}
function WYF(fun){
	...
	window.wyf_initfuns[window.wyf_initfuns.length] = fun;
}
...
function goto(url,isblank){
	$.cookie('WYFLASTPAGE',location.href);
	if(isblank){
...
jQuery.extend($.fn,{
    mask: function(msg,maskDivClass,maskcolor){
    	...
    	$('.maskdivgen').each(function(){
    		if(!hasmasked&&$(this).data('WYFMASKO')==original[0]){
    			hasmasked = true;
    			if(msg){
        ...
        maskDiv.data('WYFMASKO',original[0]);
        if(msg){
        	var msgDiv=$('<div class="WYFMASKMSG" style="border:#6593cf 1px solid; padding:2px;background:#fff;width:200px;position: absolute;"><div style="line-height:24px;border:#a3bad9 1px solid;background:white;padding:2px 10px 2px 10px;text-align:center;" id="maskmsgdiv">'+msg+'</div></div>');
      	...
      	original.data('WYFisMasked',true);
        return maskDiv;
    },
 unmask: function(){
 	this.each(function(){
 		...
 		$('.maskdivgen').each(function(){
 			if($(this).data('WYFMASKO')==original[0]){
 				$(this).next('.WYFMASKMSG').remove();
 				$(this).remove();
 			}
 		});
 		original.data('WYFisMasked',false);
 	});
...
function pophtml(html,w,h,comefrom,maskopacity,during){
	...
	$('#wyfpop').remove();
	$('#wyfmask').remove();
	var jpop = $('<div id="wyfpop" style="overflow:hidden;position: absolute;left:-1000px;top:-1000px;z-index:811201;width:'+w+'px;height:'+h+'px;"></div>');
	var jmask = $('<div id="wyfmask" style="filter:alpha(opacity=55);WIDTH: 100%;position: absolute;left:0px;top:0px;z-index:10000;background:gray;" id="wyfpop"></div>');


	jpop.append('<table id="wyfpophtmltable" cellpadding="0" cellspacing="0" border="0" width="100%"><tr><th colspan="2" height="20px;" style="background:#336699;"></th><th width="20px;" style="background:#336699;width:20px;" id="wyfpopclose"><b>╳</b></th></tr><th width="20px;"  style="background:#336699;">&nbsp;</th><td style="background-color: #dfdfdf;" height="'+(h-50)+'px" width="'+(w-40)+'px" >'+html+'</td><th width="20px;" style="background:#336699;width:20px;">&nbsp;</th></tr><tr><th colspan="3"  height="20px;" style="background:#336699;"></th></tr></table>');
	$('body').append(jpop);
	...
	window.wyfpopclose = function(){
		$('#wyfpop').find('iframe').attr('src','');
		$('#wyfpop').find('iframe').remove();
		$('#wyfpop').remove();
		$('#wyfmask').fadeOut('fast');
	...
	jpop.animate({left:(($(window).width()-w)/2)+'px',top:top+'px',width:w+'px',height:h+'px'},{duration:during,complete:function(){
		jmask.css('opacity',maskopacity/10);
		$('#wyfpophtmltable').find('th').css('opacity',0.6);
	}});

	$('#wyfpopclose').css('color','#ffffff').css('cursor','pointer').click(function(){
		window.wyfpopclose();
	});
 ...
 * @returns
 */
function WYF_getnextsel(o,tn,aw){
	var val = $(o).val();
...
function pop(oid,w,h,comefrom){
	...
	$('#wyfmask').remove();
	var jpop = $('<div id="wyfpop" style="overflow:hidden;position: absolute;left:-1000px;top:-1000px;z-index:811201;width:'+w+'px;height:'+h+'px;"></div>');
	var jmask = $('<div id="wyfmask" style="position: absolute;left:0px;top:0px;z-index:99;background:gray;" id="wyfpop"></div>');
	jmask.css('opacity',0.4);
	jpop.append('<table id="wyfpophtmltable" cellpadding="0" cellspacing="0" border="0" width="100%"><tr><th colspan="2" height="20px;" style="background:#336699;"></th><th width="20px;" style="background:#336699;" id="wyfpopclose"><b>╳</b></th></tr><th width="20px;"  style="background:#336699;"></th><td class="wyfappendtd"></td><th width="20px;" style="background:#336699;"></th></tr><tr><th colspan="3"  height="20px;" style="background:#336699;"></th></tr></table>');
	jpop.find('.wyfappendtd').append($('#'+oid).show());
	$('body').append(jpop);
	...
	jpop.animate({left:(($(window).width()-w)/2)+'px',top:top+'px',width:w+'px',height:h+'px'},{duration:600,complete:function(){
		//$(document).one('mousedown',window.wyfpopclose);
...
function tusi(txt,fun){
	$('.tusi').remove();
	var div = $('<div style="background: url('+wyf_jspath+'/img/tusi.png);max-width: 85%;min-height: 77px;min-width: 270px;position: absolute;left: -1000px;top: -1000px;text-align: center;border-radius:10px;"><span style="color: #ffffff;line-height: 77px;font-size: 23px;">'+txt+'</span></div>');
	$('body').append(div);
...
function loading(txt){
	if(txt === false){
		...
		$('.qp_lodediv').remove();
		var div = $('<div class="qp_lodediv" style="background: url('+wyf_jspath+'/img/loadb.png);width: 269px;height: 107px;position: absolute;left: -1000px;top: -1000px;text-align: center;"><span style="color: #ffffff;line-height: 107px;font-size: 23px; white-space: nowrap;">&nbsp;&nbsp;&nbsp;<img src="'+wyf_jspath+'/img/load.gif" style="vertical-align: middle;"/>&nbsp;&nbsp;'+txt+'</span></div>');
		$('body').append(div);
		div.css('zIndex',9999999);
		div.css('left',parseInt(($(window).width()-div.width())/2));
		var top = parseInt($(window).scrollTop()+($(window).height()-div.height())/2);
...
jQuery.extend(jQuery.fn,{
	validations:function(){
		//返回需要被验证的jq对象
		return this.find('[required="required"],[WYFVAL]');
		...
		var valstr = $.trim(this.attr('WYFVAL'));
		if(valstr!=''){
			...
			for(var i=0;i<valstrs.length;i++){
				...
				if(the_arr[0].indexOf('WYFUNIQUE')===0){
					//唯一性验证
...
function page_jump(fquery){
	var page = parseInt($('#WYF_PAGE_jumptxt').val());
	if(!page){
		page = 1;
	}
	$(fquery).append('<input type="hidden" name="WYF_PAGE_jumptxt" value="'+page+'">').submit();
}
