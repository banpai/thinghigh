...
class Request{
	...
	public static function get($pam=null, $default = null){
		...
		return isset($_GET[$pam]) ? trim($_GET[$pam]) : ($default === null? false : WYF::value($default));
	}
	...
	public static function post($pam=null, $default = null,$xssclean = false){
		...
		if(!isset($_POST[$pam])){
			return $default === null ? false : WYF::value($default);
		}
		if($xssclean){
	...
	public static function part_control($index = null){
		if($index === null){
			return count($_SERVER['WYF_PATH_INFO']);
		}elseif($index <0){
			return $_SERVER['WYF_PATH_INFO'][count($_SERVER['WYF_PATH_INFO'])+$index];
		}else{
			return $_SERVER['WYF_PATH_INFO'][$index];
		}
	}
