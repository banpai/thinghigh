...
class Redirect{
	...
	public static function to($url,$data = null,$status='HTTP/1.1 301'){
		...
		if($data !==null){
			Session::once('WYF_REDIRECT_DATA',$data);
		}
	...
	public static function last_data(){
		return Session::flush('WYF_REDIRECT_DATA');
	}
	...
	public static function to_404(){
		...
		include WYF_FRAME_PATH.'view/'.Conf::$view_folder.'/404'.Conf::$view_suffix;
		exit();
	}
	...
	public static function to_500($err_msg){
		...
		include WYF_FRAME_PATH.'view/'.Conf::$view_folder.'/500'.Conf::$view_suffix;
		exit();
	}
	...
	public static function prev($data = null){
		if(isset($_SESSION['WYF_HTTP_REFERER_OLD'])){
			self::to($_SESSION['WYF_HTTP_REFERER_OLD'],$data);
		}else{
			self::back();
		}
		exit();
