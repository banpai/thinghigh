...
class Index{
	...
	public function __construct(){
		if(Conf::$is_developing){
			//include WYF_LIB.'plugin/@system/compress.php';
			//$cp = new Compress();
			//$cp->cpcss();
			//$cp->cpjs();
			//@File::all_copy(WYF_FRAME_PATH.'view/'.Conf::$view_folder.'/@style/media', WYF_FRAME_PATH.WYF_PUB.'/media',true);
			//@File::all_copy(WYF_LIB.'forders/pub/@system', WYF_FRAME_PATH.WYF_PUB.'/@system',true);
		}
	}
	/**
	 * 404检查
	...
	public function _trans404() {
		//分解请求地址
		$_SERVER['WYF_PATH_INFO'] = explode('/', $this->controller_path);

		//控制器绝对路径
		$this->col_path = WYF_FRAME_PATH.'controller/'.$this->controller_path.'.php';
		if(!@file_exists($this->col_path)){
			//判断是系统控制器
			if(strpos($this->controller_path, '@system')===0){
				$this->col_path = WYF_LIB.'controller/'.$this->controller_path.'.php';
			...
			//判断是否是无控制器请求
			if(Conf::$auto_find_view&&file_exists(WYF_FRAME_PATH.'view/'.Conf::$view_folder.'/'.$this->controller_path.Conf::$suffix)){
				$this->col_path = WYF_LIB.'controller/@system/commoncontroller.php';
				return;
			}
	...
	public function _transpath() {
		...
		//视图模板路径
		$this->view_path = WYF_FRAME_PATH.'view/'.Conf::$view_folder.'/'.Page::$my_view.Conf::$view_suffix;
		if(!@file_exists($this->view_path)){
			if(isset($_SESSION['MGC_ADMIN']) && $_SESSION['MGC_ADMIN'] == 'ok'){
				$this->view_path = WYF_LIB.'sys/sysview/'.Page::$my_view.'.html';
				if(!@file_exists($this->view_path)){
					die('系统文件丢失');
				}
			}else{
		...
		//编译后的路径
		$this->com_path = WYF_FRAME_PATH.'sys/compilations/'.Page::$my_view.'.php';
	}
	/**
	 * 编译模板
	 */
	public function _trans() {
		if(Conf::$is_developing||!@file_exists($this->com_path)){
			...
			//取得包裹路径
			$this->_view_wrap_path = dirname(WYF_FRAME_PATH.'view/'.Conf::$view_folder.'/@wrap/'.Page::$my_view);
			...
			$jadapter = Page::asset('@system/js/').'wyfadapter.js';
			$appdenstr = '</title><script type="text/javascript">var wyf_jspath = "'.Page::asset('@system/').'";</script><script type="text/javascript" src="'.$jquery.'"></script><script type="text/javascript" src="'.$jadapter.'"></script>';
	...
	private function _getViewHtml($wrap_path) {
		if(file_exists($wrap_path.'/wrap'.Conf::$view_suffix)){
			...
			if(strpos($viewhtml, '<!--@NO-WRAP-->')!==false){
				...
				return str_replace('@WYF-WRAP', $viewhtml, $wraphtml);
			}
		}else{
	...
	private function _complie_string($str){
		...
		for ($i=0;$i< $Include_count;$i++){
			...
			if(strpos($viewloc, '/')===0){
				$viewloc = WYF_FRAME_PATH.'/view/'.Conf::$view_folder.$viewloc;
			}else{
		...
		for ($i=0;$i< $Include_count;$i++){
			...
			//读入相应网络内容
			$template_Conver = str_replace($Include_[0][$i],'<?php echo WYF::cache("'.$url.'"); ?>',$template_Conver);
		}
		unset($Include_);
		//对于分页的处理
		...
		while($Include_count >0){
			...
			if($pgginationsign !== null){
				...
				$pglocal = WYF_FRAME_PATH.'view/'.Conf::$view_folder.'/@pagination/'.$pgview.'/';
		...
		$template_Replace[] = '<?php echo WYF_tag_\\1; ?>';
	...
	public function check_cache() {
		...
		if(isset($_SERVER['WYF_RENEW'])){
			Cache::remove($this->cache_path.'_key');
			Redirect::to($_SERVER['WYF_RENEW']);
			return false;
		}
		...
		if(Conf::$is_developing || empty($res)){
			...
			$lines = explode('@WYF@', $res);
			$access_m = true;
			if(count($lines) > 1){
				//数据库缓存
				$dbs = explode(',', $lines[1]);
				foreach ($dbs as $db){
					if(Cache::has('WYF_TABLE_TIME'.Conf::$db_tablePrefix.$db) && intval(Cache::get('WYF_TABLE_TIME'.Conf::$db_tablePrefix.$db)) >= intval($lines[0])){
						//如果某一库表的更新时间大于等于缓存时间
						return false;
					}
				}
	...
	public function write_cache() {
		...
		if(Page::$cache_type === CACHE_NORMAL){
			$cachepath = WYF_FRAME_PATH.WYF_PUB.'/'.$_SERVER['REAL_REQUEST_URI'].'.htm';
	...
	public function _parseUrl() {
		...
		//判断是否为强制刷新
		if(strpos($url, '@WYF_RENEW') !== false){
			$url = str_replace('@WYF_RENEW', '', $url);
			$_SERVER['WYF_RENEW'] = $url;
		}
		//第一个字符是'/'，则去掉
		while($url[0]=='/'){
			$url=substr($url, 1);
