...
class WYF{
	...
	public static function i18n_page_init($path){
		$pi18npath = WYF_FRAME_PATH.'i18n/'.$_SERVER['HTTP_ACCEPT_LANGUAGE'].'/'.$path.'.php';
	...
	public static function i18n($name = ''){
		...
		if(!isset(self::$i18ns[$key])){
			self::$i18ns[$key] = include(WYF_FRAME_PATH.'i18n/'.$_SERVER['HTTP_ACCEPT_LANGUAGE'].$app.'.php');
		}
		$arrs = &self::$i18ns[$key];
	...
	public static function cache($url){
		if(strpos($url, '://')===false){
			...
			$k = base64_encode($url);
			if(!Cache::has('WYF_PUBCACHE/'.$k)){
				Cache::set('WYF_PUBCACHE/'.$k, @file_get_contents($url), Conf::$cache_time);
			}
			return Cache::get('WYF_PUBCACHE/'.$k);
		}
		return $url;
	}
}