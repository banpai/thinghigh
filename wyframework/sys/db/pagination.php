...
class Pagination{
	...
	function __construct($numperpage = 10, $linknum = 9, $showifone = true){
		...
		if(Request::get('WYF_PAGE_jumptxt')){
			$this->pagenum = intval(Request::get('WYF_PAGE_jumptxt'));
			unset($_GET['WYF_PAGE_jumptxt']);
	...
	function model_list($model,$back_array = false){
		$db = DB::get_db();
		$sql = $db->com_sql($model->WYFSYS_real_tablename,$model->WYFSYS_condition,null,null,"count(*) c",$model->WYFSYS_pam);
