...
class SampleModel{
	/**是否原生模型*/
	public $WYFSYS_isorgin = true;
	/**表名*/
	public $WYFSYS_tablename;
	/**数据符合规范的标识*/
	public $WYFSYS_val_fail = false;
	/**主键*/
	public $id;
	/**存贮字段的待选值数组 */
	public static $WYFSYS_FIELD_DATA = array();
	/**存贮字段的描述数组 */
	public static $WYFSYS_FIELD_LABLE = array();
	/**存贮字段的类型数组 */
	public static $WYFSYS_FIELD_TYPE = array();
	/**存贮字段的默认值数组 */
	public static $WYFSYS_FIELD_DEFAULT = array();
	/**存贮字段是否允许非空的数组 */
	public static $WYFSYS_FIELD_CANNULL = array();
	/**存贮字段Form验证字串的数组 */
	public static $WYFSYS_FIELD_FORMVAL = array();
	/**存贮字段的验证错误信息数组 */
	public $WYFSYS_field_error = array();
	/**Form提交的加密后的区别标志*/
	protected  $WYFSYS_post_id = '';
	/**Form提交的区别标志*/
	protected $WYFSYS_postid = '';
	/**是否是初始的验证信息设置*/
	public static $WYFSYS_first_valset = array();
	...
	public function load_from_post(){
		...
		$newpost = array();
		if(Page::$tk_str != null){
			$prevstr = $this->WYFSYS_tablename.'T'.$this->WYFSYS_postid;
			foreach ($_POST as $k=>$v){
				$newpost[String::decryption($k,Page::$tk_str )] = $v;
			}
		}else{
			$prevstr = $this->WYFSYS_tablename.'T'.$this->WYFSYS_post_id;
			$newpost = $_POST;
		}
		Page::$tk_str = null;

		$begin = strlen($prevstr);
		if(!empty($newpost[$prevstr.'id'])&&isset($this->WYFSYS_db)){
	...
	public function load_from_get(){
		$prevstr = $this->WYFSYS_tablename.'T'.$this->WYFSYS_post_id;
		$begin = strlen($prevstr);
		if(!empty($_GET[$prevstr.'id'])&&isset($this->WYFSYS_db)){
	...
	public function load_all_from_post(){
		$prevstr = $this->WYFSYS_tablename.'T';
		$begin = strlen($prevstr);
		...
		foreach ($posts as $k=>$post){
			if(!$this->WYFSYS_isorgin){
				$mod = new $this->WYFSYS_tablename(k);
			}else{
				$mod = new model($this->WYFSYS_tablename,k);
			}
			if(!empty($post['id'])&&isset($this->WYFSYS_db)){
				$mod->find($post['id']);
			}
	...
	public function set_err_msg($field,$errmsg){
		if(!isset($this->WYFSYS_field_error[$field])){
			$this->WYFSYS_field_error[$field] = array();
		}
		$this->WYFSYS_field_error[$field][] = $errmsg;
		$this->WYFSYS_val_fail = true;
	}
	/**
	 * 获得模型的验证错误信息数组<br/>
	 * 这是一个以字段名称为数组下标的二维数组
	...
	public function errors(){
		return $this->WYFSYS_field_error;
	}
	/**
	 * 获得模型的验证第一个错误消息
	 * 没有则返回空字符串
	...
	public function field_error($field){
		...
		return $this->WYFSYS_field_error[$field];
	}
	/**
	...
	public function field_errors($field){
		...
		return implode(',', $this->WYFSYS_field_error[$field]);
	}
	/**
	 * 显示错误信息
	 */
	...
	public function val_tel($field,$errmsg=null){
		$this->val_reg($field,"/(\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$/", $errmsg===null?WYF::i18n('validate.tel'):$errmsg);
	}
	/**
	 * 字段国内手机合法性验证
	 * @param string $field 字段名称
	...
	public function val_phone($field,$errmsg=null){
		$this->val_reg($field,"/^0{0,1}(13[0-9]|15[0-9]|17[0-9]|18[0-9]|90[0-9])[0-9]{8}$/", $errmsg===null?WYF::i18n('validate.phone'):$errmsg);
	}
	/**
	 * 字段Email格式合法性验证
	 * @param string $field 字段名称
	...
	public function val_email($field,$errmsg=null){
		$this->val_reg($field,"/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/", $errmsg===null?WYF::i18n('validate.email'):$errmsg);
	}
	/**
	 * 字段url合法性验证
	 * @param string $field 字段名称
	...
	public function val_url($field,$errmsg=null){
		$this->val_reg($field,"/^(http|https):\/\/[^\s]*$/", $errmsg===null?WYF::i18n('validate.url'):$errmsg);
	}
	/**
	 * 字段非空验证
	 * @param string $field 字段名称
	...
	public function val_notnull($field,$errmsg=null){
		if(!$this->set_field_required_string($field,null, $errmsg===null?WYF::i18n('validate.null'):$errmsg)){
			if(trim($this->$field)==''){
				$this->set_err_msg($field,$errmsg?$errmsg:WYF::i18n('validate.null'));
			}
		}
	}
	/**
	...
	public function val_number($field,$errmsg=null){
		...
		$this->val_reg($field,"/^\d+\.?\d*$/", $errmsg===null?WYF::i18n('validate.number'):$errmsg);
	}
	/**
	 * 字段是否为整数的验证
	 * @param string $field 字段名称
	...
	public function val_integer($field,$errmsg=null){
		...
		$this->val_reg($field,"/^(0|[1-9][0-9]*)$/", $errmsg===null?WYF::i18n('validate.integer'):$errmsg);
	}
	/**
	 * 字段是否为中文的验证
	 * @param string $field 字段名称
	...
	public function val_chinese($field,$errmsg=null){
		$this->val_reg($field,"/^[\x{4e00}-\x{9fa5}\s]+$/u", $errmsg===null?WYF::i18n('validate.chinese'):$errmsg,'/^[\u4e00-\u9fa5]+$/i');
	}
	/**
	 * 字段是否为英文和数字的验证
	 * @param string $field 字段名称
	...
	public function val_english($field,$errmsg=null){
		$this->val_reg($field,"/^[1-9a-zA-Z\s]+$/", $errmsg===null?WYF::i18n('validate.english'):$errmsg);
	}
	/**
	 * 字段最小长度的验证
	 * @param string $field 字段名称
	...
	public function val_min_length($field,$min,$errmsg=null){
		//$this->val_reg($field,"/^.{".$min.",}$/", str_replace('{0}', $min, $errmsg===null?WYF::i18n('validate.short'):$errmsg));
		$regjs = "/^.{".$min.",}$/";
		//非空验证通过才执行其他验证
		if(!$this->set_field_required_string($field, $regjs, $errmsg)&&$this->$field!=''&&mb_strlen($this->$field)<$min){
			$this->set_err_msg($field,$errmsg===null?WYF::i18n('validate.long'):$errmsg);
		}
	}
	/**
	 * 字段最大长度的验证
	...
	public function val_max_length($field,$max,$errmsg=null){
		//$this->val_reg($field,"/^.{0,".$max."}$/", str_replace('{0}', $max, $errmsg===null?WYF::i18n('validate.long'):$errmsg));
		$regjs = "/^.{0,".$max."}$/";
		//非空验证通过才执行其他验证
		if(!$this->set_field_required_string($field, $regjs, $errmsg)&&$this->$field!=''&&mb_strlen($this->$field)>$max){
			$this->set_err_msg($field,$errmsg===null?WYF::i18n('validate.long'):$errmsg);
		}
	}
	/**
	...
	public function val_min($field,$min,$errmsg=null){
		...
		if($num<$min){
			$this->set_err_msg($field,$errmsg===null?WYF::i18n('validate.min'):$errmsg);
		}
	}
	/**
	 * 最大值校验
	...
	public function val_max($field,$max,$errmsg=null){
		$num = floatval($this->$field);
		if($num>$max){
			$this->set_err_msg($field,$errmsg===null?WYF::i18n('validate.max'):$errmsg);
		}
	}
	/**
	 * 输出hidden标签
	...
	public function hidden($name,$attrs=''){
		...
		if(is_array($attrs)){
			...
			$attrs = WYF_get_attrs_from_array($attrs);
		}
	...
	function vercode($local,$width=120,$height=45,$attrs='',$ischinese=false,$codenum=4){
		...
		$elname = $this->elname('WYF_vercode');
		$id = $this->elid('WYF_vercode');
		...
		$html .= '<script>';
		$html .= '$(function(){$("#'.$id.'").blur();$("#'.$id.'").focus(function(){ if(!$("#wyf_verimg").is("img")){';
		$html .= 'var verimg = $(\'<img id="wyf_verimg" title="点击切换" style="width:'.$width.'px;height:'.$height.'px;" />\');';
	 ...
	 */
	function vercodeok(){
		if(isset($this->WYF_vercode)){
			return strtolower($this->WYF_vercode) == strtolower(Session::get('WYF_vercode'));
		}else{
			return true;
		}
	}
	...
	public function field_form_name($name){
		return $this->WYFSYS_tablename.'T'.$this->WYFSYS_post_id.$name;
	}
	/**
	 * 不同type的input标签调用
	...
	public function color($name,$attrs='',$needal = false){
		...
		$tag = '<div name="Alpha_'.$this->elid($name).'" id="Alpha_'.$this->elid($name).'"></div><input type="hidden" needal="'.$needal.'" colorid="Alpha_'.$this->elid($name).'" relobj="wyfcolor" rel="wyf" value="'.$value.'" id="'.$this->elid($name).'" name="'.$this->elname($name).'" />';
		return $tag;
	}
	...
	public function elid($name){
		return $this->WYFSYS_tablename.$this->WYFSYS_postid.$name;
	}
	...
	public function elname($name){
		if(Page::$tk_str != null){
			return String::encryption($this->WYFSYS_tablename.'T'.$this->WYFSYS_postid.$name,Page::$tk_str);
		}else{
			return $this->WYFSYS_tablename.'T'.$this->WYFSYS_post_id.$name;
		}
	}
	...
	public function mulselect($tn,$name_arr,$attrs='',$addwhere='',$impstr='',$autotext = true){
		...
		foreach ($name_arr as $index=>$name){
			...
			if($index<($arr_len-1)){
				...
				$change = '"WYF_getnextsel(this,\''.String::encryption($ntn).'\',\''.String::encryption($addwherestr).'\')"';
			...
			$tag.='<option value="" >'.WYF::i18n('.select_please').'</option>';
			if($pid != null){
				$m = new Model($ttn);
				$beg = "pid='".$pid."'";
				if($nopid){
	...
	function texteditor($name,$level=3,$width='640px',$height='300px'){
		...
		return $tag.'<input type="hidden" relobj="kindeditor" rel="wyf" editorid="'.$this->elid($name).'" value="'.$opts.'" />';
	}
	/**
	 * 输出日期时间标签
	 * @param string $name 字段名称 标签name
	...
	function datetime($name,$attrs='',$isdate = true){
		...
		$tag = '<input type="text" '.$this->field_required_string($name).' value="'.$datevalue.'"  id="'.$this->elid($name).'" '.$attrs.' name="'.$this->elname($name).'" onfocus="WdatePicker({ dateFmt:\'yyyy-MM-dd HH:mm:ss\'})" relobj="wyfcalendar" rel="wyf"/>';
		return $tag;
	}
	/**
	 * 输出日期标签
	...
	function date($name,$attrs='',$isdate = true){
		...
		$tag = '<input type="text" '.$this->field_required_string($name).' value="'.$datevalue.'"  id="'.$this->elid($name).'" '.$attrs.' name="'.$this->elname($name).'" onfocus="WdatePicker({ dateFmt:\'yyyy-MM-dd\'})" relobj="wyfcalendar" rel="wyf"/>';
		return $tag;
	}
	...
	function mdate($name,$attrs='',$isdate = true){
		...
		$tag = '<input type="text" '.$this->field_required_string($name).' value="'.$datevalue.'"  id="'.$this->elid($name).'" '.$attrs.' name="'.$this->elname($name).'" mdate="date" relobj="wyfmcalendar" rel="wyf"/>';
		return $tag;
	}
	...
	function mdatetime($name,$attrs='',$isdate = true){
		...
		$tag = '<input type="text" '.$this->field_required_string($name).' value="'.$datevalue.'"  id="'.$this->elid($name).'" '.$attrs.' name="'.$this->elname($name).'" mdate="datetime" relobj="wyfmcalendar" rel="wyf"/>';
		return $tag;
	}
	/**
	...
	public function field_required_string($field){
		$valstr = " ";
		if(self::$WYFSYS_FIELD_CANNULL[$this->WYFSYS_tablename][$field]===false){
			$valstr .= 'required="required" ';
		}
		if(!$this->WYFSYS_isorgin){
			//自定义的构造
			if(isset(self::$WYFSYS_FIELD_FORMVAL[$this->WYFSYS_tablename])&&isset(self::$WYFSYS_FIELD_FORMVAL[$this->WYFSYS_tablename][$field])){
				$tvalstr = self::$WYFSYS_FIELD_FORMVAL[$this->WYFSYS_tablename][$field];
				$tvalstr = str_replace('@WYFSYSID', '@'.$this->id, $tvalstr);
				$valstr .= 'WYFVAL="'.htmlspecialchars($tvalstr,ENT_QUOTES).'" ';
			}
		}
		return $valstr;
	}
	...
	public function set_field_required_string($field,$reg,$msg){
		if($reg===null){
			//需要非空的信息写入
			self::$WYFSYS_FIELD_CANNULL[$this->WYFSYS_tablename][$field] = false;
		}elseif(self::$WYFSYS_first_valset[$this->WYFSYS_tablename]){//第一次执行
			if(!isset(self::$WYFSYS_FIELD_FORMVAL[$this->WYFSYS_tablename][$field])){
				self::$WYFSYS_FIELD_FORMVAL[$this->WYFSYS_tablename][$field] = '';
			}
			self::$WYFSYS_FIELD_FORMVAL[$this->WYFSYS_tablename][$field] = $reg.'REG@MSG'.$msg.'ONE@ANOTHER'.self::$WYFSYS_FIELD_FORMVAL[$this->WYFSYS_tablename][$field];
		}
		return self::$WYFSYS_first_valset[$this->WYFSYS_tablename];
	}
}
?>