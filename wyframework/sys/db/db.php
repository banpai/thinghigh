...
class DB{
	...
	public function base_query($sql,$pam=null) {
		if(is_array($sql)){
			...
			if(Conf::$need_db_log){
				//开发模式下记录db日志
				file_put_contents(WYF_FRAME_PATH.'/log/db/'.date('Y_m_d_H',time()).'.log', date('i:s',time()).','.self::$current_db.': '.$sql."\n",FILE_APPEND);
			}
			//调试日志
			...
			if(self::$dbdes  !== null){
				...
				if(self::$dbdes->operate !='R'){
					Cache::set('WYF_TABLE_TIME'.self::$dbdes->real_table,time());
				}
			...
			return $this->statement;
			 ...
			 if(is_string($pam)){//参数为字符串类型
			 	$sql = str_replace('@wYf_SqL@',"'".mysql_escape_string($pam)."'",$sql);
			 }else if(is_array($pam)){//单纯数组
			 	$sqlss = explode('@wYf_SqL@', $sql);
			 	$sqllen = count($pam);
			 	$sql = '';
			 	for($i=0;$i<$sqllen;$i++){
			 		if($pam[$i]===null){
			 			$sql.=($sqlss[$i]."null");
			 		}elseif(strpos($pam[$i], '@wYf_RaW@') === 0){
			 			$sql.=($sqlss[$i].substr($pam[$i], 9));
			 		}else{
	...
	function fill_models($model,$condition=null,$order=null,$limit=null,$field='*',$pam=null){
		$sql = $this->com_sql($model->WYFSYS_real_tablename,$condition,$order,$limit,$field,$pam);
		$res = array();
		$query = $this->base_query($sql);
		$modelname = $model->WYFSYS_tablename;
		if(!$model->WYFSYS_isorgin){//如果是用户的扩展类
			while($tn=mysql_fetch_array($query,MYSQL_ASSOC)){
	...
	function fill_one_model(& $model,$condition,$pam=null,$ord=null,$limit=null){
		$sql = $this->com_sql($model->WYFSYS_real_tablename,$condition,$ord,$limit,$model->WYFSYS_select,$pam);
		$query = $this->base_query($sql);
	...
	function update_entity($table,$condition=null,$data=array(),$pam=null){
		...
		if(!empty($condition)){
			if(is_string($condition)){
				$condition = str_replace('?', '@wYf_SqL@', $condition);
			}
	...
	function get_fields_array($table){
		if($table != '' && empty(self::$tablefieldsarray[$table])){//jinye 修改即使表名为空仍然会调用show full fields引起error的问题
	...
	public function com_sql($table,$where=null,$order=null,$limit=null,$field='*',$pam=null){
		...
		if(!empty($where)){
			if(is_string($where)){
				$where = str_replace('?', '@wYf_SqL@', $where);
			}
