...
class SampleModel{
	 ...
	 * @param string $errmsg 错误信息
	 */
	public function val_eamil($field,$errmsg=null){
	 ...
	 * @return string 标签html字串
	 */
	function vercode($local,$attrs='',$ischinese=false,$codenum=4,$width=120,$height=45){
		if(is_array($attrs)){
			$attrs = WYF_get_attrs_from_array($attrs);
		}
		...
		$html = '<input type="text" required="required"  name="'.$elname.'" id="'.$id.'" '.$attrs.'/>';
		$html .= '<script>';
	 ...
	 */
	function vercodeok(){
		return $this->WYF_vercode == Session::get('WYF_vercode');
	}
	/**
	 * 输出password标签
	 * @param string $name 字段名称 标签name
	 ...
	 * @return string 标签html字串
	 */
	public function select($array,$name='',$attrs=''){
		$additionstr = '"';
		if(is_string($array)){
			$attrs = $name;
			$name = $array;
		...
		$tag = '<select '.$this->field_required_string($name).' name="'.$this->elname($name).$additionstr.'  id="'.$this->elid($name).'" '.$attrs.'>';
	...
	public function mulselect($tn,$name_arr,$attrs='',$addwhere='',$impstr=''){
		...
		foreach ($name_arr as $index=>$name){
			$value = $this->$name;
			$onchange = $index<($arr_len-1) ? 'onchange="WYF_getnextsel(this,\''.String::encryption($tn).'\',\''.String::encryption($addwherestr).'\')"': '';
	...
	public function checkbox($array,$name='',$attrs=''){
		if(is_string($array)){
			...
			foreach ($array as $k=>$v){
				$k=strval($k);
				$tag.=$v.':<input type="checkbox" name="'.$this->elname($name).'[]" value="'.htmlspecialchars($k).'" '.((in_array($k, $value,true))? 'checked="checked"' : '').' '.$attrs.'/>&nbsp;&nbsp;';
			}
			return $tag;
		}
	...
	public function radio($array,$name='',$attrs=''){
		...
		$tag = '';
		foreach ($array as $k=>$v){
			$tag.=$v.':<input type="radio" name="'.$this->elname($name).'" value="'.htmlspecialchars($k).'" '.(($k==$value)? 'checked="checked"' : '').' '.$attrs.'/>&nbsp;&nbsp;';
		}
		return $tag;
	}
	/**
	...
	function texteditor($name,$level=3,$width='640px',$height='300px'){
		...
		if(!is_array($level)){
			$level = intval($level);
			$allowupload = ($level==3||$level==4||$level==6||$level==7)?'true':'false';
	...
	function datetime($name,$attrs='',$isdate = true){
		...
		$tag = '<input type="text" value="'.$datevalue.'" '.$this->field_required_string($name).' id="'.$this->elid($name).'" '.$attrs.' onfocus="wyfcalendar.initCalendar(this,true,$(this).next(\'input\')[0],'.$isdate.')"/><input type="hidden" relobj="wyfcalendar" rel="wyf" value="'.$datevalue.'" name="'.$this->elname($name).'" />';
		return $tag;
	}
	/**
	 * 输出日期标签
	...
	function date($name,$attrs='',$isdate = true){
		...
		$tag = '<input type="text" '.$this->field_required_string($name).' value="'.$datevalue.'"  id="'.$this->elid($name).'" '.$attrs.' onfocus="wyfcalendar.initCalendar(this,false,$(this).next(\'input\')[0],'.$isdate.')"/><input type="hidden" relobj="wyfcalendar" rel="wyf" value="'.$datevalue.'" name="'.$this->elname($name).'" />';
		return $tag;
	}
