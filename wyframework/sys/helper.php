...
function tk(){
	$tkid = uniqid();
	if(!isset($_SESSION['WYF_FORM_TOKEN'])){
		$_SESSION['WYF_FORM_TOKEN'] = array();
	}
	$tk_arr = &$_SESSION['WYF_FORM_TOKEN'];
	$tk_arr[$tkid] = microtime(true);
	Page::$tk_str = $tk_arr[$tkid].'@WYF@'.$tkid;
	return '<input type="hidden" value="'.$tkid.'" name="WYF_FORM_TOKEN"/>';
}
