...
public static function init($name,$config=null,$data=null,$url=null,$picareaid=null,$cutpic=null){
	...
	$ttid = 'WYF_'.md5('WYF_UPLOAD_'.$name.microtime());
	$surl = $url;
	$width = $config['width'];
	...
	$htm = '';
	if(is_array($cutpic)){
		$htm = '<input type="hidden" relobj="wyfcutpic" rel="wyf" />';
		$url = '/@system/upload.html';
...
<input name="WYF_UPLOAD_file" style="position: absolute;height: 100%;width:{$width}px;right: 0;z-index:2" type="file"/>
...
<input type="hidden" name="WYF_UPLOAD_SIZE" value="{$size}"/>
<input type="hidden" name="WYF_UPLOAD_ID" value="{$ttid}"/>
EOT;
//隐参添加
foreach ($data as $k=>$v){
...
$htm .= <<<EOT
	...
	var {$ttid}pic = $('#{$ttid}').find('input[name="WYF_UPLOAD_file"]');
...
static function save_upload_file($subpath, $funupload=null,$funcut=null){
	//临时目录清空
	File::remove_dir(WYF_FRAME_PATH.WYF_PUB.'/upload/temp/',(time()-86400));
	File::creat_dir_with_filepath(WYF_FRAME_PATH.WYF_PUB.'/'.$subpath);
	if(!empty($_FILES)&&isset($_FILES['WYF_UPLOAD_file'])){
		//第一次上传
		$thisid = $_POST['WYF_UPLOAD_ID'];
		$thissize = intval($_POST['WYF_UPLOAD_SIZE'])*1000;
		$f = $_FILES['WYF_UPLOAD_file'];
		if($f['size']>$thissize){
			...
			if (!move_uploaded_file($f['tmp_name'], WYF_FRAME_PATH.WYF_PUB.'/'.$filename)) {
				Response::write('<script>window.parent.'.$thisid.'=1;window.parent.'.$thisid.'_failure("server error",4);</script>');
			}else{
				if($funupload !==null){
					call_user_func_array($funupload,array(WYF_FRAME_PATH.WYF_PUB.'/'.$filename));
				}
		...
		if(!file_exists(WYF_FRAME_PATH.WYF_PUB.'/'.$filename)){
			return false;
		}
		//剪切之后的处理
		$img_r = imagecreatefromstring(file_get_contents(WYF_FRAME_PATH.WYF_PUB.'/'.$filename));
		if($img_r===false){
			Response::write('no');
		}
		$dsw = Request::post('w');
		...
		imagejpeg($dst_r,WYF_FRAME_PATH.WYF_PUB.'/'.$filename2,90);
		if($filename2!=$filename){
			unlink(WYF_FRAME_PATH.WYF_PUB.'/'.$filename);
		}
		if($funcut !==null){
			call_user_func_array($funcut,array(WYF_FRAME_PATH.WYF_PUB.'/'.$filename2));
		}
...
private static function _resave_upload_file($key,$folderpath=null,$isFile = false){
	if(trim($key)==''){
		...
		//删除上次的文件地址
		@unlink(WYF_FRAME_PATH.WYF_PUB.'/'.substr($key, 4));
		return false;
	}elseif (strpos($key, '@-@')!==false){
		//修改后删除上次的文件地址
		$keys = explode('@-@', $key);
		$key = $keys[0];
		@unlink(WYF_FRAME_PATH.WYF_PUB.'/'.$keys[1]);
	}
	if($folderpath===null){
		$folderpath = 'upload/auto/'.date('Y/m/d',time());
	}
	if(!$isFile){
		File::creat_dir(WYF_FRAME_PATH.WYF_PUB.'/'.$folderpath);
		$tsubppath = '/'.$folderpath.'/'.basename($key);
		$newpath = WYF_FRAME_PATH.WYF_PUB.'/'.$folderpath.'/'.basename($key);
	}else{
		File::creat_dir_with_filepath(WYF_FRAME_PATH.WYF_PUB.'/'.$folderpath);
		$tsubppath = '/'.$folderpath;
		$newpath = WYF_FRAME_PATH.WYF_PUB.'/'.$folderpath;
	}
	@unlink($newpath);
	rename(WYF_FRAME_PATH.WYF_PUB.'/'.$key, $newpath);
