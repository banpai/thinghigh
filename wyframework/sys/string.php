...
class String{
	...
	public static function random($length = 6, $pool = ''){
		...
		for($i = 0; $i < $length; $i++){
			$mt = mt_rand();
			$random .= substr($pool,($mt%(strlen ($pool))), 1);
		}
		return $random;
	}

	/**
	 * jsonencode，	其中的utf8字符不进行转义
	 *
	 * @param	array	$arr	数组
	 * @return	string
	 */
	public static function jsonencode_unescape($arr){
		$str = json_encode($arr);
