...
class Cache{
	...
	public static function set($k, $v, $time = 0){
		...
		if(Conf::$cache_adapter == 'memcached'){
			...
			$path = WYF_FRAME_PATH.'sys/cache/'.str_replace('%2F', '/', urlencode($k));

			File::creat_dir_with_filepath($path);
			file_put_contents($path, ($time === 0 ? '0' : (time()+ intval($time))).'@WYF_CACHE@'.serialize($v), LOCK_EX);
		}
	}
	...
	public static function forever($k, $v = null){
		$path = WYF_FRAME_PATH.'sys/cache/WYFFOREVER/'.str_replace('%2F', '/', urlencode($k));
		if($v === null){
			//读取
			if(file_exists($path)){
	...
	public static function forget($k){
		return File::remove_file_with_parentdir(WYF_FRAME_PATH.'sys/cache/'.str_replace('%2F', '/', urlencode($k)));
	}
	/**
	 * 获得缓存的值
	 *
	...
	public static function get($k){
		if(Conf::$cache_adapter == 'memcached'){
			...
			$path = WYF_FRAME_PATH.'sys/cache/'.str_replace('%2F', '/', urlencode($k));
			if(file_exists($path)){
				$res = explode('@WYF_CACHE@', file_get_contents($path));
