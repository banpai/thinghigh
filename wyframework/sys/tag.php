...
$COM = WYF::i18n();
/**具体页面国际化*/
$TXT = WYF::i18n_page_init(Page::$my_view);
/**
 * select 标签
 ...
 */
function WYF_tag_select(&$array,$name,$attrs=''){
	$nameid = $name;
	global $$name;
	$value = $$name;
	if(is_array($attrs)){
 ...
 * @param mixed $attrs 其他属性可以是字符或者数组
 */
function WYF_tag_checkbox($array,$name='',$attrs=''){
	if(is_string($array)){
		$name = $array;
		$attrs = $name;
		$nameid = $name;
		$value = WYF_getNameValue($name);
		if(is_array($attrs)){
			$attrs = WYF_get_attrs_from_array($attrs);
		}
		if($value=='1'){
 ...
 * @param mixed $attrs 其他属性可以是字符或者数组
 */
function WYF_tag_input($type,$name,$attrs=''){
	$nameid = $name;
	$value = WYF_getNameValue($name);
	if(is_array($attrs)){
		$attrs = WYF_get_attrs_from_array($attrs);
 ...
 * @param mixed $attrs 其他属性可以是字符或者数组
 */
function WYF_tag_text($name,$attrs=''){
	return WYF_tag_input('text', $name,$attrs);
}
/**
 * input password 标签
 * @param string $name 属性值
 * @param mixed $attrs 其他属性可以是字符或者数组
 */
function WYF_tag_password($name,$attrs=''){
	return WYF_tag_input('password', $name,$attrs);
}
/**
 * input email 标签
 * @param string $name 属性值
 * @param mixed $attrs 其他属性可以是字符或者数组
 */
function WYF_tag_email($name,$attrs=''){
	return WYF_tag_input('email', $name,$attrs);
}
/**
 * input range 标签
 * @param string $name 属性值
 * @param mixed $attrs 其他属性可以是字符或者数组
 */
function WYF_tag_range($name,$attrs=''){
	return WYF_tag_input('range', $name,$attrs);
}
/**
 * 根据数组拼接标签属性
 * @param $array
 * @return string 属性字串
 */
function WYF_get_attrs_from_array(&$array){
	$attrs = '';
	foreach ($array as $k=>$v){
 ...
 * @param mixed $attrs 其他属性可以是字符或者数组
 */
function WYF_tag_radio($array,$name,$attrs=''){
	$nameid = $name;
	$value = WYF_getNameValue($name);
	if(is_array($attrs)){
		$attrs = WYF_get_attrs_from_array($attrs);
 ...
 * @param mixed $attrs 其他属性可以是字符或者数组
 */
function WYF_tag_hidden($name,$attrs=''){
	$nameid = $name;
	$value = WYF_getNameValue($name);
	if(is_array($attrs)){
		$attrs = WYF_get_attrs_from_array($attrs);
 ...
 * @param mixed $attrs 其他属性可以是字符或者数组
 */
function WYF_tag_textarea($name,$attrs=''){
	$nameid = $name;
	$value = WYF_getNameValue($name);
	if(is_array($attrs)){
		$attrs = WYF_get_attrs_from_array($attrs);
 ...
 * @param mixed $attrs 其他属性可以是字符或者数组
 */
function WYF_tag_datetime($name,$attrs=''){
	$nameid = $name;
	$value = WYF_getNameValue($name);
	if(is_array($attrs)){
		$attrs = WYF_get_attrs_from_array($attrs);
	...
	$tag = '<input type="text" value="'.$datevalue.'"  id="'.$nameid.'" '.$attrs.' onfocus="wyfcalendar.initCalendar(this,true,$(this).next(\'input\')[0])"/><input type="hidden" relobj="wyfcalendar" rel="wyf" value="'.$value.'" name="'.$nameid.'" />';
	return $tag;
}
/**
 * input text 日期标签
 ...
 * @param mixed $attrs 其他属性可以是字符或者数组
 */
function WYF_tag_date($name,$attrs=''){
	$nameid = $name;
	$value = WYF_getNameValue($name);
	if(is_array($attrs)){
		$attrs = WYF_get_attrs_from_array($attrs);
	...
	$tag = '<input type="text" value="'.$datevalue.'"  id="'.$nameid.'" '.$attrs.' onfocus="wyfcalendar.initCalendar(this,false,$(this).next(\'input\')[0])"/><input type="hidden" relobj="wyfcalendar" rel="wyf" value="'.$value.'" name="'.$nameid.'" />';
	return $tag;
}
/**
 * input text 颜色标签
 ...
 * @param mixed $attrs 其他属性可以是字符或者数组
 */
function WYF_tag_color($name,$needal = true){
	$nameid = $name;
	$value = WYF_getNameValue($name);
	if(is_array($attrs)){
		$attrs = WYF_get_attrs_from_array($attrs);
	...
	$tag = '<div name="Alpha_'.$nameid.'" id="Alpha_'.$nameid.'"></div><input type="hidden" needal="'.$needal.'" colorid="Alpha_'.$nameid.'" relobj="wyfcolor" rel="wyf" value="'.$value.'" id="'.$nameid.'" name="'.$nameid.'" />';
	return $tag;
}
/**
 * texteditor 标签
 ...
 */
function WYF_tag_texteditor($name,$level=3,$width='640px',$height='300px'){
	$nameid = $name;
	$value = WYF_getNameValue($name);
	global $wyf_filedname_index;
	$namerealid = str_replace('[]', '', $name);
	$namerealid = $namerealid.trim($wyf_filedname_index[$namerealid]);
	...
	return $tag.'<input type="hidden" relobj="kindeditor" rel="wyf" editorid="'.$namerealid.'" value="'.$opts.'" />';
}
/**
 * 验证码标签
 ...
 */
function WYF_tag_vercode($text,$local,$sound=false,$width=115,$height=30,$codenum=4){
	$YYSYS = Page::asset('@system/');
	$html = '<script>';
	$html .= '$(function(){$("#'.$text.'").data("mbl",1);$("#'.$text.'").blur();$("#'.$text.'").data("mbl",0);$("#'.$local.'").append(\'<input type="hidden" id="wyf_verimg_tag"/>\');$("#'.$text.'").focus(function(){ if(!$("#wyf_verimg").is("img")){';
	$html .= 'var verimg = $(\'<img id="wyf_verimg" title="双击切换" />\');';
 ...
 */
function WYF_getNameValue($name){
	if(strpos($name, '[]')==false){
		...
		if(is_array($realvar)){
			global $wyf_filedname_index;
			if(empty($wyf_filedname_index)){
				$wyf_filedname_index = array();
			}
			if(!isset($wyf_filedname_index[$name])){
				$wyf_filedname_index[$name] = 0;
			}
			$res = htmlspecialchars(trim($realvar[$wyf_filedname_index[$name]]),ENT_QUOTES);
			$wyf_filedname_index[$name] = $wyf_filedname_index[$name] + 1;
			return $res;
		}
	}
}
